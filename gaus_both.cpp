...
void determinant_seq(int n,float **a){
  int i,j,k;
...
int main()
{
  ...
  //  cout<<"\nThe matrix after Pivotisation is:\n";
	...
	{
		cout<<"determinant value:"<<a[0][0];
                return 0;
	}
  if(n == 2)
  {
      cout <<"determinant value:"<<(a[0][0] * a[1][1] - a[1][0] *a[0][1]);
      return 0;
   }
   else
    {
   cout<<"\n The sequential method to calculate the det of the matrix:\n";
  struct timeval start1 , end1 ;
   gettimeofday(&start1,NULL);                  //getting the system time before executing the Sequential method
   determinant_seq(n,a_s);
   gettimeofday(&end1,NULL);

   // cout<<"\n\nThe matrix after gauss-elimination using sequential method is as follows:\n";
   //  for (i=0;i<n;i++)            //print the new matrix
   //  {
   //      for (j=0;j<n;j++)
   //          cout<<a[i][j]<<setw(16);
  ...
  cout<<"\n\n The determinant using the sequential is: "<<dets << endl;

  cout<<"\n\n The parallel method to calculate the det of the matrix:\n";
